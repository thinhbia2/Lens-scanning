...
class IntensityMapGUI:
    ...
    def setup_variables(self):
        ...
        self.fitting_options = ["Raw", "Subtract Background", "Subtract Average", "Subtract Slope", "Subtract Linear Fit", "Subtract Parabolic Fit"]
        ...
        self.fitting_methods = {
            ...
            "Subtract Background": twoDfittings.subtract_background,
    ...
    def setup_controls(self):
        ...
        self.save_button = tk.Button(self.root, text="Save Data", bg="blue", font=self.arr18, command=self.save_intensity_maps)
        ...
        if filename:
            ...
            if self.dropdown_var.get() == "Height":
                with open(filename + '_raw_z.txt', 'w', newline='') as f1:
                    ...
                    writer1.writerows(self.raw_intensity1)
                with open(filename + '_processed_z.txt', 'w', newline='') as f2:
                    writer2 = csv.writer(f2, delimiter='\t')
                    writer2.writerows(self.im1.get_array())
                # Save images
    ...
    def save_image(self, data, cmap, vmin, vmax, output_filename):
        fig = plt.figure(figsize=(3+0.5, 3))

        spec = gridspec.GridSpec(1, 2, width_ratios=[3, 0.15], wspace=0)
        ax = fig.add_subplot(spec[0])
        cax = fig.add_subplot(spec[1])
        im = ax.imshow(data, cmap=cmap, vmin=vmin, vmax=vmax, aspect="equal")  # Keep square aspect
    ...
    def update_z_plot(self, x, y, intensity_value):
        if self.is_running:
            ...
            if (x == frame_size-1 and y % 2 == 0) or (x == 0 and y % 2 == 1):
                fitted_data1 = self.fitting_methods.get(self.fitting1.get(), twoDfittings.raw)(self.raw_intensity1)
                self.im1.set_data(fitted_data1)
